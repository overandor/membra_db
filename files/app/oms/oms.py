...
from app.risk.risk import risk, Fill
...
class OMS:
    ...
    async def cancel_all_exchange(self, session: aiohttp.ClientSession) -> None:
        """Cancel all open orders on the exchange."""
        from app.connectors.rest_client import cancel_all_orders
        async with self._lock:
    ...
    async def clear_local_state(self) -> None:
        """Clear local order state without touching the exchange."""
        async with self._lock:
            for s in self._state.values():
                s.bid_order_id = s.ask_order_id = None
                s.bid_price = s.ask_price = None
                s.bid_size = s.ask_size = 0
                s.bid_placed_ts = s.ask_placed_ts = 0.0
        log.info("Local order state cleared")
    ...
    async def cancel_all(self, session: aiohttp.ClientSession) -> None:
        ...
        Use cancel_all_exchange() followed by clear_local_state() instead.
        """
        await self.cancel_all_exchange(session)
        await self.clear_local_state()
    ...
    @property
    def state_(self):
        """Public access to internal state for API."""
        return self._state

    def get_or_create(self, contract: str) -> ContractOrders:
        """Public getter for state."""
        return self._get_or_create(contract)
