...
@router.post("/network/agents/{agent_id}/task")
async def execute_task(agent_id: str, req: TaskRequest):
    reg = _get_registry()
    agent = reg.get(agent_id)
    if not agent:
        raise HTTPException(status_code=404, detail=f"Agent {agent_id} not found")
    result = await agent.execute_task(req.task_type, req.params)
    return {
        "agent_id": agent_id,
        "full_name": agent.full_name,
...
def get_peers(agent_id: str):
    ...
    if not agent:
        raise HTTPException(status_code=404, detail=f"Agent {agent_id} not found")
    return {"agent_id": agent_id, "peers": agent.peers, "peer_count": len(agent.peers)}
