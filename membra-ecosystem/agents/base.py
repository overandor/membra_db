"""MEMBRA Agent OS — Base Agent Class.
Every employee is an autonomous agent with:
  ...
  - Job-specific skills (real methods matching their title)
...
MEMBRA_NETWORK_PORT = int(os.getenv("MEMBRA_NETWORK_PORT", "0"))
...
class BaseAgent(ABC):
    ...
    async def think(self, prompt: str, context: Optional[str] = None) -> str:
        """Run a thought through Ollama. All agents use this for reasoning."""
        self.status = "busy"
    ...
    # ─── Job-Specific Skill Methods (Override in subclasses) ───
    async def execute_task(self, task_type: str, params: Dict[str, Any]) -> Dict[str, Any]:
        """Execute a job-specific task. Each agent overrides this with real skills."""
        # Default: delegate to LLM
        prompt = f"Execute {task_type} with params: {json.dumps(params)}"
    ...
    def _on_rpc(self, msg: AgentMessage) -> Optional[AgentMessage]:
        ...
        if method == "status":
            ...
            # Run job-specific task asynchronously
            return self._reply(msg, {"ack": True, "note": "Use 'task' msg_type for execution"})
    ...
    async def _on_task(self, msg: AgentMessage) -> Optional[AgentMessage]:
        ...
        params = msg.payload.get("params", {})
        result = await self.execute_task(task_type, params)
